...
def test_emoji_checker():
    ...
    assert_eq('lines_checked',
              summary.arbitrary_data['test'].get('lines_checked'), 61)
    assert_eq('lines_skipped',
              summary.arbitrary_data['test'].get('lines_skipped'), 4)
...
def test_toc_checker():
    ...
    taken_redirect_summary = extra_summary_data[3]
    not_in_toc_summary = extra_summary_data[4]
    broken_hover_img_path_summary = extra_summary_data[5]
    broken_part_img_path_summary = extra_summary_data[6]
    ...
    taken_redirect_count = taken_redirect_summary.count('.md') / 2
    assert_eq('taken redirect count', taken_redirect_count, 1)
